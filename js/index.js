...
function shortenUrl() {
    ...
    if (flag) {
        ...
        return swal.fire({
            icon: "info",
            title: "You are not using a valid URL!",
            width: "auto",
        });
}

// Event listener for "Shorten URL" button click
shortButton.addEventListener("click", (e) => {
    e.preventDefault();
    shortenUrl();
});

// Event listener for Enter key press on originalUrl input
originalUrl.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        e.preventDefault();
        shortenUrl();
    }
